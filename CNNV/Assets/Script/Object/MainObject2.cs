...
public class MainObject2 : MonoBehaviour
{
    ...
    void CreateImage()
    {
        String path = "Assets/Resources/Textures/MNIST/matrix_data.mat";
        if (!System.IO.File.Exists(path)) { return;  }
        try
        {
            Matrix<float> m = MatlabReader.Read<float>(path, "image");
            CreateCubes(m, 0, 0, 1);
        }
        catch
        {
    ...
    void UpdateImage()
    {
        ...
        {
            Matrix<float> m = MatlabReader.Read<float>(path, "image");
            isUpdate = lastData[0, 0] == null || !WhetherSame(lastData[0, 0], m);
            UpdateCubes(m, 0, 0, 1);
        }
        catch
        {
    ...
    void CreateNeural(int[] data)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            Vector3 position = new Vector3(20 * (i - 4.5f), 0f, (dataSize.Length + 2) * 100f);
        ...
        try
        {
            ...
            for (int i = 0; i < edge; i++)
            {
                ...
                {
                    Vector3 position = new Vector3(getRandom(ran, -100, 100), getRandom(ran, -100, 100), getRandom(ran, 280, 320));
        ...
        try
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    {
                        objs[dataSize.Length + 2, i, 0, 0].GetComponent<MeshRenderer>().material.color = new Color(1f, 1f, 1f);
            ...
            else
            {
                ...
                for (int i = 0; i < edge; i++)
                {
                    for (int j = 0; j < edge; j++)
                    {
                        ...
                        {
                            objs[dataSize.Length + 1, 0, i, j].GetComponent<MeshRenderer>().material.color = new Color(1f, 1f, 1f, 0.5f);
                        }
                    }
                }
            }
                ...
                for (int i = 0; i < 20; i++)
                {
                    for (int j = 0; j < 20; j++)
                    {
                        for (int k = 0; k < 10; k++)
                        {
                            ...
                            if ((int)(objs[dataSize.Length + 1, 0, i, j].GetComponent<MeshRenderer>().material.color.g + 1e-6) == 0
                                && (int)(objs[dataSize.Length + 2, k, 0, 0].GetComponent<MeshRenderer>().material.color.g + 1e-6) == 0)
                            {
                                //Debug.Log("11111");
                                lines[(i * 20) + j, k].GetComponent<LineRenderer>().material.color = new Color(1f, 0f, 0f);
